...
def test_update_identity_providers(equal_endpoint: bool) -> None:
    ...
    current_issuers = [copy.deepcopy(old_issuer)]
    update_identity_providers(current_issuers=current_issuers, new_issuer=new_issuer)
    if equal_endpoint:
        ...
        assert len(current_issuers[1].user_groups) == len(old_issuer.user_groups)
